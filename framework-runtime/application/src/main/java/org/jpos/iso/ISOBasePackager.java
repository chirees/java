				...
				if ((c = (ISOComponent) fields.get(i)) != null) {
					try {
						...
						if (logger != null) {
							...
							evt.addMessage("<pack fld=\"" + i + "\" packager=\"" + fld[i].getClass().getName() + "\" name=\"" + fld[i].getDescription() + "\">");
			...
			for (int i = getFirstField(); i < maxField; i++) {
				try {
					...
					if (bmap == null || bmap.get(i)) {
						...
						if (logger != null) {
						    String value= c.getValue()+"";
							evt.addMessage("<unpack fld=\"" + i + "\" packager=\"" + fld[i].getClass().getName() + "\" name=\"" + fld[i].getDescription() + "\">");
							if (c.getValue() instanceof ISOMsg)
								evt.addMessage(c.getValue());
							else if (c.getValue() instanceof byte[]) {
								evt.addMessage("  <value type='binary'>" + ISOUtil.hexString((byte[]) c.getValue()) + "</value>");
			...
			if (bmap != null && bmap.get(65) && fld.length > 128 && fld[65] instanceof ISOBitMapPackager) {
				...
				for (int i = 1; i < 64; i++) {
					if (bmap == null || bmap.get(i)) {
						...
						if (logger != null) {
							evt.addMessage("<unpack fld=\"" + i + 128 + "\" packager=\"" + fld[i + 128].getClass().getName() + "\">");
							evt.addMessage("  <value>" + c.getValue().toString() + "</value>");
							evt.addMessage("</unpack>");
						}
						m.set(c);
