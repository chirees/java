    ...
    */
    ...
    public static byte[] str2bcd(String s, boolean padLeft, byte[] d, int offset) {
        ...
        for (int i=start; i < len+start; i++) 
            d [offset + (i >> 1)] |= (asc_to_bcd(s.charAt(i-start))) << ((i & 1) == 1 ? 0 : 4);
        return d;
    }
    /**
     * converts to BCD
    ...
    public static byte[] str2bcd(String s, boolean padLeft, byte fill) {
        ...
        int start = (((len & 1) == 1) && padLeft) ? 1 : 0;
        for (int i=start; i < len+start; i++) 
            d [i >> 1] |= (asc_to_bcd(s.charAt(i-start))) << ((i & 1) == 1 ? 0 : 4);
        return d;
    }
     ...
     * positions the decimal separator must be moved from the right. Positions 2鈥� of
