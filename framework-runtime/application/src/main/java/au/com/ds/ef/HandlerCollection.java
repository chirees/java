...
class HandlerCollection {
    ...
    public <C extends StatefulContext> boolean callOnEventTriggered(EventEnum event, StateEnum stateFrom, StateEnum stateTo, C context) throws Exception {
        ...
        if (h != null) {
            ...
            return eventHandler.call(event, stateFrom, stateTo, context);
        }
        
        return true;
    }
    
    public <C extends StatefulContext> boolean callOnEventFailed(EventEnum event, StateEnum stateFrom, StateEnum stateTo, C context) throws Exception {

        Handler h = handlers.get(new HandlerType(EventType.EVENT_FAIL, null, null));
        if (h != null) {
            EventHandler<C> eventHandler = (EventHandler<C>) h;
            return eventHandler.call(event, stateFrom, stateTo, context);
        }
